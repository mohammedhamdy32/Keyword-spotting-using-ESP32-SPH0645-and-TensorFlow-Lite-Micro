...
#if ( USED_PSRAM == 1 )
  uint8_t *g_arena = (uint8_t *) heap_caps_malloc( kArenaSize , MALLOC_CAP_SPIRAM ) ; 
#else
                                   ...
                                   tflite::MicroInterpreter* interpreter) 
{
  ...
  TfLiteTensor* input = interpreter->input(0); /* The input size is 2D array, one is a wrapper, and other is a 480size, it's datatype is int16 */
  // printf("%d %d\n" , input->dims->data[0] , input->type );
  ...
  std::copy_n(tflite::GetTensorData<int8_t>(output), g_kFeatureSize,
              feature_output);
