...
bool MfccMelFilterbank::Initialize(int input_length, double input_sample_rate,
  ...
  if (num_channels_ < 1) {
    std::cout << "Number of filterbank channels must be positive.";
    return false;
  }

  if (sample_rate_ <= 0) {
    std::cout << "Sample rate must be positive.";
    return false;
  }

  if (input_length < 2) {
    std::cout << "Input length must greater than 1.";
    return false;
  }

  if (lower_frequency_limit < 0) {
    std::cout << "Lower frequency limit must be nonnegative.";
    return false;
  }

  if (upper_frequency_limit <= lower_frequency_limit) {
    std::cout << "Upper frequency limit must be greater than " << "lower frequency limit.";
    return false;
  }
  ...
  if (!bad_channels.empty()) {
    std::cout  << "Missing " << bad_channels.size() << " bands "
...
void MfccMelFilterbank::Compute(const std::vector<double> &input,
  ...
  if (!initialized_) {
    std::cout << "Mel Filterbank not initialized.";
    return;
  }

  if (input.size() <= (size_t)end_index_) {
    std::cout << "Input too short to compute filterbank";
    return;
  }
