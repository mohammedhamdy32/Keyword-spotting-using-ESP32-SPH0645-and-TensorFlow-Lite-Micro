...
bool MfccDct::Initialize(int input_length, int coefficient_count) {
  ...
  if (coefficient_count_ < 1) {
    std::cout << "Coefficient count must be positive.";
    return false;
  }

  if (input_length < 1) {
    std::cout << "Input length must be positive.";
    return false;
  }

  if (coefficient_count_ > input_length_) {
    std::cout << "Coefficient count must be less than or equal to " << "input length.";
    return false;
  }
